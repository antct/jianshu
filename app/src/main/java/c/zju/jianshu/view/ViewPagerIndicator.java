...
public class ViewPagerIndicator extends HorizontalScrollView {
    ...
    public ViewPagerIndicator(Context context, AttributeSet attrs) {
        ...
        // 获取XML中的配置属性
        TypedArray attributes = context.obtainStyledAttributes(attrs, c.zju.jianshu.R.styleable.ViewPagerIndicator);
        mTabVisibleCount = attributes.getInt(c.zju.jianshu.R.styleable.ViewPagerIndicator_tab_visible_count, COUNT_DEFAULT_TAB);
        if (mTabVisibleCount < 0) {
            mTabVisibleCount = COUNT_DEFAULT_TAB;
        }
        mSizeText = attributes.getInt(c.zju.jianshu.R.styleable.ViewPagerIndicator_tab_text_size, SIZE_TEXT);
        mColorTextNormal = attributes.getInt(c.zju.jianshu.R.styleable.ViewPagerIndicator_tab_text_normal_color, COLOR_TEXT_NORMAL);
        mColorTextHighlight = attributes.getInt(c.zju.jianshu.R.styleable.ViewPagerIndicator_tab_text_highlight_color, COLOR_TEXT_HIGHLIGHT);
        mColorLine = attributes.getInt(c.zju.jianshu.R.styleable.ViewPagerIndicator_tab_line_color, COLOR_LINE);
        mHeightLine = attributes.getInt(c.zju.jianshu.R.styleable.ViewPagerIndicator_tab_line_height, HEIGHT_LINE);
        attributes.recycle();
